...
*   **Pricing waitlist persists to Postgres via `/api/waitlist` (Brief 11.2).** Replaces the old formsubmit.co relay. Route at `artifacts/api-server/src/routes/waitlist.ts` writes to the `waitlist_signups` table (Drizzle schema in `lib/db/src/schema/waitlist.ts`, unique index on `email`). `onConflictDoNothing` makes resubmits return `{ok:true, duplicate:true}` so the UI shows a friendly "already on the list" message instead of an error. Frontend at `artifacts/promptmegood/pricing.html` L405-468 — payload is just `{email, source}`; no FormSubmit `_subject`/`_template`/`_honey` fields. To add a new field, update the Drizzle table → `pnpm --filter @workspace/db run push` → extend `insertWaitlistSignupSchema`. Abuse controls: (1) Cloudflare Turnstile widget on the form, verified server-side via siteverify when `TURNSTILE_SECRET_KEY` is set — when the secret is unset (e.g. local dev) the verification is skipped so the form still works; site key is exposed via `/api/public-config` as `turnstileSiteKey`. (2) Per-IP rate limiter (5 signups / 10 min) via `makeRateLimiter`. (3) Emails are NOT logged in plaintext — only a non-reversible `emailFp` fingerprint is logged for burst correlation. Required secrets for prod: `TURNSTILE_SITE_KEY` (browser-safe, sent via public-config) + `TURNSTILE_SECRET_KEY` (server-only). Query the table via `select * from waitlist_signups order by created_at desc`.
