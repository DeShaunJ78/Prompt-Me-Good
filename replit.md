...
*   **Cache-buster:** Bump `?v=cv2-N` on both `pmg-chassis-v2.css` and `pmg-chassis-v2.js` script/link tags in `app.html` whenever you change either file. Currently `cv2-8`.
...
*   **Result panel is always visible:** Do NOT hide `#result-panel` based on `.has-result` — that class is never set on the panel. The "has result" signal lives on `body.pmg-has-result` (set by `pmg-ux.js:306`). Hiding the panel based on a never-matched selector swallows the user's generated prompt and makes the app look broken.
