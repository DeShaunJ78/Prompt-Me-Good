...
*   `artifacts/promptmegood/public/styles/` + `public/scripts/`: Workstation runtime stylesheets and scripts.
    ...
    *   `pmg-business-mode.{css,js}` — Business Mode 4th panel: Brand Voice Vault + 6 workflow packs (18 templates total) + dynamic variable form + Generate Master Prompt + quality-check toggle.
...
*   **Cache-busters (current):** `pmg-chassis-v3.css` `t140-image-mode-removed`, `pmg-chassis-v3.js` `t140-image-mode-removed`, `pmg-ux.js` `t140-image-mode-removed`, `pmg-pro.js` `t140-image-mode-removed`, `pmg-image-extras.js` `t140-image-mode-removed`, `pmg-share.js` `t140-image-mode-removed`, `pmg-handoff.js` `t140-image-mode-removed`, `pmg-visual-studio.{css,js}` `ps-2-build-above-fold`, `pmg-storyboard.css` `sb-7`, `pmg-storyboard.js` `sb-9`, `pmg-auto-boost.css` `ab-5-always-show`, `pmg-auto-boost.js` `ab-5-always-show`, `pmg-spark-panel.{css,js}` `sp-3-typewriter`, `pmg-business-mode.{css,js}` `bm-1`, `pmg-send-to.js` `sendto-4-silent`, `pmg-templates.js` `tpl-1`. Brand assets `?v=5`. Bump only what changed.
*   **Legacy image-mode path removed (Task #140):** `window.setMode`, `window.runImageGeneration`, `#modeSwitch`, `#writeModeBtn`/`#imageModeBtn`, `.image-mode-hint`, `#image-generate-btn`, `pmg-ux.js` `reorderImageMode()`, plus the in-page `setMode`/`runImageGeneration` IIFE and the image-mode tour step are all GONE. `body.image-mode` toggling SURVIVES — chassis-v3 `setActivePanel('photography')` adds it (and removes it on text/video) because the relocated Photo Suite's legacy CSS depends on the class. `pmg-share.js` `applyMode()` and `pmg-handoff.js` text↔image handoffs now route via `window.pmgChassisV3.setActivePanel('photography'|'text')` instead of the removed `window.setMode`. Other lingering `window.setMode` references (pmg-undo wrapper, pmg-suite-handoff cue mount) are guarded by `typeof === 'function'` and are inert no-ops. `pmg-image-extras.js` `wrapGenerator()` retry is bounded to 30 ticks (~6s) so the wrapper gives up cleanly when `runImageGeneration` is absent. To enter image mode programmatically, call `window.pmgChassisV3.setActivePanel('photography')`.
...
*   **Session persistence (cv3-49 / cv3-55):** Idea + tuning + generated prompt persist to `sessionStorage.pmgv3:session` (7-day defensive TTL) via `wirePersistence()` in `pmg-chassis-v3.js`. **`sessionStorage`, NOT `localStorage`** — survives switching apps / opening Gemini in a new tab (the original goal: don't lose mid-task work when iOS Safari unloads the backgrounded tab), but auto-clears on full tab close so users get a clean slate on next fresh open. Saves debounced 400ms on `#goal` input + tuning select changes + `#resultBox` MutationObserver + `visibilitychange`/`pagehide`/`beforeunload`. Restore on boot re-fills goal, dispatches `change` on selects, writes prompt back to `#resultBox`, reveals `#prompt-output-box` + `#run-with-ai-btn`, hides `#analyze-btn` (post-analyze surface). `doStartOver()` suspends persistence for 700ms and clears the session twice. `writeSession()` skips when both goal AND prompt are empty — default tuning alone is NOT user signal (otherwise pagehide on a freshly reset page re-writes junk). Disable hatches: `?fresh=1`, `localStorage.pmgv3_persist_disable='1'` (the disable flag stays on `localStorage` so it persists across tabs).
