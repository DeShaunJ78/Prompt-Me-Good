...
*   **Cache-buster:** Bump `?v=cv2-N` on `pmg-chassis-v2.css` and `pmg-chassis-v2.js` in `app.html` on changes. Currently `cv2-30`. `pmg-visual-studio.{css,js}` at `vs-10`; `pmg-storyboard.{css,js}` at `sb-4`. Brand assets (`pmg-logo.png`, `favicon-32.png`, `favicon-48.png`) use `?v=5`.
