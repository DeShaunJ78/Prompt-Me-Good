...
*   **Cache-buster:** `pmg-auto-boost.css` + `pmg-auto-boost.js` at `ab-1`. `pmg-chassis-v3.css` at `cv3-32`, `pmg-chassis-v3.js` at `cv3-31`. `pmg-visual-studio.css` + `pmg-visual-studio.js` at `vs-20-image-workshop`; `pmg-storyboard.css` at `sb-7`, `pmg-storyboard.js` at `sb-8`. `pmg-ux.js` at `cv3-28-camera-angle`. Brand assets (`pmg-logo.png`, `favicon-32.png`, `favicon-48.png`) use `?v=5`.
...
*   **Topbar icon tap targets:** `.pmgv3-ico` enforces a 44×44 min tap target on touch devices via `@media (hover: none) and (pointer: coarse)`. The visual glyph stays at 32×32 (≤400px) / 36×36 (desktop) but `min-width`/`min-height` expands the hit area on phones.
