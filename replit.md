...
-   **T41 Stripe Checkout (Subscriptions):** "Upgrade To Pro" button on `pricing.html` (PRO tier card) and an auto-injected inline CTA card above `#pmg-account` on the homepage, all wired by the `T41` IIFE at the bottom of `pmg-ux.js` via `[data-pmg-upgrade]`. Clicks call `POST /api/create-checkout-session` (api-server) using a Bearer JWT pulled from the live T40 Supabase session, then redirect to Stripe Checkout. Webhook is the **source of truth** — `POST /api/stripe-webhook` (mounted in `app.ts` BEFORE `express.json()`, with `express.raw({type:'application/json'})` so signatures verify) writes plan/status/period to a Supabase `profiles` table. Frontend syncs by polling `GET /api/me/profile` and mirroring `plan==='pro'` into the existing `pmg-pro` localStorage cache (`promptmegood:pro:v1`); after `?upgrade=success` it polls every 2s for 30s and shows a "Payment Confirmed" toast. Hardening: every Supabase response is `{error}`-checked (throws → 500 → Stripe retries); return URLs come from a strict allowlist (`REPLIT_DOMAINS` + canonical `*.promptmegood.com`), not the raw `Origin`; Stripe customer creation uses `idempotencyKey: pmg-customer-<uid>` to defeat duplicate-customer races; `STRIPE_WEBHOOK_SECRET` and `STRIPE_SECRET_KEY` validated at boot. Required Replit Secrets: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_ID`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`. SQL + Stripe webhook setup steps in `artifacts/promptmegood/SUPABASE-SETUP.md` sections 3–4. pmg-pro.js was NOT modified.
