...
*   **Cache-buster:** `pmg-auto-boost.css` + `pmg-auto-boost.js` at `ab-3`. `pmg-chassis-v3.css` at `cv3-36`, `pmg-chassis-v3.js` at `cv3-37`. `pmg-visual-studio.css` + `pmg-visual-studio.js` at `vs-22-photo-accordion`; `pmg-storyboard.css` at `sb-7`, `pmg-storyboard.js` at `sb-9`. `pmg-ux.js` at `cv3-28-camera-angle`. Brand assets (`pmg-logo.png`, `favicon-32.png`, `favicon-48.png`) use `?v=5`.
