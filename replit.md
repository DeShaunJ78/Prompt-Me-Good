...
-   **UX Polish Pass (May 2026):** Standardized text-flow CTA to **Fix My Prompt** across How-It-Works steps, post-uc/tip-block guidance, Prompt Tuning helper, keyboard-shortcut hint, and the image-mode "needs prompt" alert; the image flow keeps a distinct **Generate Image** label. Added inline examples under the text **Your Goal** field (`<span class="helper-example">`) and the `.image-mode-hint` (italic “Try…” snippet). Repositioned **Help Me Start** copy as beginner rescue: *"Best for beginners — answer 4 quick questions and we'll build it with you. Skip if you already know what to type."* Added discoverable tooltip `title=` attributes to **More Detailed / More Bold & Direct / Beginner Friendly** plus a static `.refine-preset-legend` (mobile-friendly, three short lines). Made post-result actions persistent in `.actions-row.pmg-result-actions-row`: added **⬇ Download .txt** (`#download-prompt-btn`, blob-based filename `promptmegood-<slug>-<YYYY-MM-DD>.txt`) alongside Copy Prompt / Print / Clear, and a **✓ Saved To Vault** pill (`#pmg-vault-saved-indicator`) that toggles via a `MutationObserver` on `#resultBox`. Image-result row now shows **⬇ Download Image / Regenerate / Copy Prompt** (and the existing `pmg-ux.js` T35 Use As Reference pill). Updated Prompt Vault subhead to *"Stored on this device. Export anytime."* with explicit cloud-sync-only-with-Founding wording. All wiring lives in a single additive inline IIFE near the bottom of `index.html` guarded by `window.__pmgPolishActionsLoaded`, with `dataset.pmgBound='1'` to prevent double-binding and a `document.execCommand('copy')` fallback for the image-prompt copy. No visual identity, color, layout, typography, or aesthetic changes.
