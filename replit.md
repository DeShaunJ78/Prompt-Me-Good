...
### Universal close affordances

Every modal, overlay, `<details>` panel, toast, and the mobile nav can be closed via at least one of: × button, ESC, backdrop click, or selection.

The `pmgCloseAffordances` block in `pmg-ux.js` exposes a small `register(el, opts)` helper that tags an element with `data-pmg-modal` and wires the × button, backdrop click, and ESC according to the kind (`native-dialog` or `div-overlay`). For overlays that already own a close handler (compare, backup), the × delegates to the existing close button so state cleanup is preserved. Toast, global search dropdown, mobile nav, and long `<details>` panels each have a small dedicated wiring function. All registrations are idempotent.
...
-   **Examples Surface Removed:** The hero "See What It Can Do / Real Examples, Real Results" CTA, the entire "Popular Uses" use-case cards section (`#use-cases` with `.popular-use-card` buttons + `🎲 Need Ideas?` dice button), the in-builder `.examples-block` quick-start chips, the `#usecases-modal` "Choose A Starting Point" picker dialog, the `setupUseCasesModalAndDice` IIFE (~205 lines, including the curated `POOL` of 39 idea starters and re-roll logic), the Smart Assist "Use Cases" Stage-2 button + `openUseCases()` helper, the `applyPopularUseCard` helper + `window.__pmgApplyPopularUseCard` global, and the `.uc-confirm-toast` "starting idea was added" banner (HTML, CSS, JS) were all removed from `index.html`. The post-tour banner copy was updated to point at Transform Text Pro instead of Use Cases. The corresponding JS handlers in `pmg-ux.js` were deleted (no orphan no-ops): `injectUsecasesScrollHint`, `polishHeroSecondaryCta`, `fixRealExamplesCta`, `moveNeedIdeasUnderCarousel`, `hideExamplesCTA`, `wirePhotoCards`, `wireUseCaseCards`, the T46 `.popular-use-card` click handler, and all related call sites. CSS rules for `.popular-uses`, `.popular-use-card`, `.usecases-dice-btn`, `.uc-modal*`, `#hero-usecases-cta`, `#use-cases`, `.examples-block`, `.example-chip`, `.use-cases-helper-tip`, T28 examplesCTA reframe, T15.6 need-ideas-wrap, and the Fix-2 mobile use-cases reorder were all deleted. The `guide.html` "Use Cases" / "Get Ideas Instantly" sections + TOC links + "Need Ideas?" + "example chips" mentions were removed.
