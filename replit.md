...
-   **Upload Field Reorder + Collapse (Task #98 — feed context BEFORE submit):** User flagged via screenshot that the `#upload-field` dropzone ("Add A File Or Image (Optional)") was rendering AFTER the Fix My Prompt button. The upload's purpose is to feed context INTO the prompt — analyzes a file via OpenAI and injects the extracted content into the goal/details — so position-after-submit was actively misleading and disrupted the build flow. Cause: T39 (premium-polish IIFE in `index.html`) reorders the form as Goal → Settings → Fix My Prompt → Upload → Guidance, and `pmg-linear-flow.js` sets `#upload-field { order: 4 }`. New T98 IIFE (`pmgT98UploadCollapse`) appended to `pmg-ux.js` is strictly additive: (a) bumps `#upload-field` flex order from 4 to 1 (same bucket as Goal textarea); selector `body:not(.image-mode) #prompt-form > #upload-field#upload-field` uses duplicate id qualifier to beat linear-flow's `:not` rule by id-count specificity; (b) collapses dropzone by default to a small inline trigger pill ("📎 Add A File Or Image (Optional) ▾") with neutralized border/padding/background; (c) tap pill removes `pmg-t98-collapsed`, lands focus on `#analyze-file` (the file input — keyboard-operable: Space/Enter opens picker) with fallback to `upload-btn-label` if input.focus fails, toggle auto-hides when expanded; (d) auto-expands when `#upload-preview`'s `hidden` attribute flips (mutation observer covers drop/paste/programmatic paths) plus dragover capture-listener for early expansion; (e) idempotency guard (`watchInstalled` boolean) ensures observer + dragover listener install exactly once across the three `init()` re-runs (DOMContentLoaded + T+600 + T+1500). Verified visual order at runtime: Goal(2121px) → Upload(2331px) → Settings(2426px) → Submit(4979px). Cache buster bumped to `?v=task98-upload-collapse`.
-   **Most Loved Badge Clearance (Task #97 — fix wrapped-text overlap):** User-reported screenshot showed the gold "MOST LOVED" pill badge on the Help Me Start button (`#pmg-help-me-start-btn`) overlapping the first line of wrapped button label — the "k" of "Quick Questions" was obscured at narrow widths. Root cause: T24 set the absolute-positioned badge at `top:-10px` and `pmg-linear-flow.js` overrode it to `top:-7px` in text mode; with the badge ~22px tall, it dipped 12–15px INTO the button. Invisible on a single-line button (text centered) but collided with text when the label wrapped. New T97 IIFE (`pmgT97MostLovedClearance`) appended to `pmg-ux.js` injects a CSS override lifting the badge to `top:-22px right:10px`. Selector specificity bumped via duplicate class qualifier (`.pmg-help-me-start-btn.pmg-help-me-start-btn .pmg-hms-most-loved.pmg-hms-most-loved`) so it beats `pmg-linear-flow.js`'s `:not(.image-mode)` rule by class count and source order. Strictly additive — no IDs/classes/handlers changed. Verified at 1029px width: badge bottom (1999.75px) sits at button top (1999.16px) with 0.59px residual overlap (essentially zero). Cache buster bumped to `?v=task97-most-loved-clearance`.
