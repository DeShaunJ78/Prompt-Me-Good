...
*   **Cache-buster:** Bump `?v=cv2-N` on `pmg-chassis-v2.css` and `pmg-chassis-v2.js` in `app.html` on changes. Currently `cv2-44`. `pmg-chassis-v3.css` at `cv3-16`, `pmg-chassis-v3.js` at `cv3-15`. `pmg-visual-studio.{css,js}` at `vs-14`; `pmg-storyboard.{css,js}` at `sb-6`. Brand assets (`pmg-logo.png`, `favicon-32.png`, `favicon-48.png`) use `?v=5`.
*   **cv3-13 Form rescue (CRITICAL):** chassis-v2.js reparents `#prompt-form` INTO `#pmg-chassis-v2-root`. v3's `suppressLegacyChassisV2()` then deletes that root — which previously took the form (and its submit listener bound at app.html ~L8843) with it, leaving `forms found: 0` and breaking generation entirely. Fix: `rescueFormBeforeRemoval()` evacuates `#prompt-form` to `<body>` (with `display:none !important`) BEFORE removing the v2 root, AND the MutationObserver re-runs the rescue every tick to catch v2 re-mounting. Also added a defensive "stray form to body" pass in the same observer. The form's submit listener is bound to the form element itself, so moving the form preserves the listener; `getFormData()` reads inputs via global `getElementById` so reparented children stay readable. cv3-14 click handler calls `form.requestSubmit()` since the reparented `#generateBtn` is no longer a form descendant and HTML5 form-attribute association proved unreliable for submit dispatch in this combo.
