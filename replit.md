...
-   **Image Generation UX (NEW):** `pmg-image-fix.js` (loaded after pmg-fixes-v6.js) layers UX over the existing `runImageGeneration()` flow without renaming any IDs. Adds: (1) a live progress pill `#pmg-image-progress` with a per-second elapsed counter and rotating reassurance copy during the 15–25s OpenAI image API wait, (2) a green success callout `#pmg-image-success` above the download button, (3) restyled prominent download button (`⬇ Save Image To Your Device`, full-width on mobile, brief pulse, dynamic timestamped filename), (4) a `#pmg-image-retry` button on errors, (5) a MutationObserver on `#imageResultWrap` that resets the generate-button label the moment an `<img>` arrives (fixes a v6 bug where the CTA stayed stuck on "Creating Your Image With DALL·E 3..." indefinitely because v6 only watched `#result-panel`). Idempotency-guarded via `data-pmg-image-fix-bound`; cleans timers on `pagehide`/`beforeunload`.
