...
*   **Cache-buster:** Bump `?v=cv2-N` on `pmg-chassis-v2.css` and `pmg-chassis-v2.js` in `app.html` on changes. Currently `cv2-44`. `pmg-chassis-v3.css` at `cv3-10`, `pmg-chassis-v3.js` at `cv3-15`. `pmg-visual-studio.{css,js}` at `vs-14`; `pmg-storyboard.{css,js}` at `sb-6`. Brand assets (`pmg-logo.png`, `favicon-32.png`, `favicon-48.png`) use `?v=5`.
