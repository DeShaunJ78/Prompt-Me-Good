...
*   **Cache-buster:** `pmg-auto-boost.css` + `pmg-auto-boost.js` at `ab-2`. `pmg-chassis-v3.css` at `cv3-33`, `pmg-chassis-v3.js` at `cv3-31`. `pmg-visual-studio.css` + `pmg-visual-studio.js` at `vs-20-image-workshop`; `pmg-storyboard.css` at `sb-7`, `pmg-storyboard.js` at `sb-8`. `pmg-ux.js` at `cv3-28-camera-angle`. Brand assets (`pmg-logo.png`, `favicon-32.png`, `favicon-48.png`) use `?v=5`.
...
*   **Topbar icon tap targets:** `.pmgv3-ico` enforces a 44×44 min tap target via TWO media queries — `@media (hover: none) and (pointer: coarse)` for real touch devices AND `@media (max-width: 768px)` as a fallback that catches devtools mobile emulation, narrow desktop windows, and browsers that mis-report pointer type. Visual glyph stays at 32×32 (≤400px) / 36×36 (desktop); only the hit area grows.
