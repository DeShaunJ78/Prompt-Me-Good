...
*   `artifacts/promptmegood/public/styles/pmg-visual-studio.css` + `public/scripts/pmg-visual-studio.js`: Visual Studio modal (image + video prompt builder).
...
*   **Cache-buster:** Bump `?v=cv2-N` on `pmg-chassis-v2.css` and `pmg-chassis-v2.js` in `app.html` on changes. Currently `cv2-29`. Brand assets (`pmg-logo.png`, `favicon-32.png`, `favicon-48.png`) use `?v=5`.
