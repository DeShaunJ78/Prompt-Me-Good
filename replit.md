...
*   **ECC Apply visibility (ecc-apply-visibility-1):** All three ECC primary "Apply ___" buttons (Diagnose / Architect / Tune) call `applyAndClose()` instead of `closeDrawer()`. `applyAndClose` closes ECC AND closes the surrounding Tune Prompt overlay if open (`window.pmgTuneChips.close(false)`) so the user actually sees the rewritten `#goal`. **Does NOT auto-fire Build** — the user reviews the rewrite and chooses Run With AI, send-to, or Build manually. Without the overlay-close, users opening ECC from inside Tune saw "nothing happen" because the Tune overlay still covered the screen after Apply.
...
*   **Modal overlay (tc-9p):** Clicking the pill calls `openOverlay()` which mounts (lazy) `#pmg-tune-overlay` (backdrop) + `#pmg-tune-overlay-panel` (modal) at `<body>` level — both `data-pmg-overlay-root` so chassis universal-hide doesn't erase them. `#tuning-panel` is reparented from its original left-column home into `.pmg-tune-overlay-body`; original parent + nextSibling are stashed on first open and reused for `closeOverlay()`. Body scroll is locked via `body.style.overflow='hidden'` + `body.pmg-tune-overlay-open` class. Esc key + backdrop click close WITHOUT firing Build; the sticky footer's `#pmg-tune-overlay-done` ("✓ Done — Build My Prompt") closes AND fires `#generateBtn.click()` after a 60ms tick (no second click needed). Same code path on mobile and desktop — no width-fork, no relocation safety nets, no top-vs-bottom button competition.
*   **Secondary in-section Done bar:** `injectDoneButton()` still appends `.pmg-tune-done-bar` (`#pmg-tune-done` + `#pmg-tune-ecc-link`) inside `.tuning-section` for users who scroll the overlay body — the always-visible overlay footer is the primary, the in-section bar is a polite belt-and-suspenders. The bar's sticky positioning is overridden inside the overlay (overlay footer is the real sticky CTA).
*   **Panel-switch auto-close:** MutationObserver on `.pmgv3-body[data-active-panel]` (NOT `document.body` — chassis writes the attribute on the inner `.pmgv3-body`). Switching from Text → Photography/Video calls `closeOverlay(false)` so users don't come back into a stale overlay.
*   **Legacy Build button hide-on-build (tc-9p + double-cta-1):** CSS rule hides `#generateBtn` on either `body.pmg-has-result` (post-result) OR `body.pmg-has-generated` (added synchronously on the first Build click). Earlier tc-9p only covered post-result, so the giant button stayed visible during the streaming phase beneath the open Tuning section — users reported "the generate prompt button is still there beneath it". double-cta-1 closes that visual gap. The right-column action grid (Run With AI, Auto-Boost, Edit, Rewrite, Send to ChatGPT) provides every next-step CTA. Reopening the overlay + clicking Done re-fires the build (the click still works on a `display:none` element). Removes the "redundant Generate My Prompt button" friction.
*   **Desktop section max-height (cv3-62):** `.tuning-section` cap of `calc(100vh - 120px)` on screens ≥769px is irrelevant inside the overlay (overlay body has its own scroll) but still applies if the section ever renders in its original spot.
