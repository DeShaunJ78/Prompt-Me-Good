...
*   **Cache-busters:** Bump query string on changed `pmg-*.{css,js}` (current image-mode-cleanup buster: `t140-image-mode-removed`; current visual-studio buster: `mux-2-adv-tuning`; current chassis-v3 buster: `mux-3-whisperer-demoted`). Brand assets `?v=5`.
