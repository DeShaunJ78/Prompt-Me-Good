...
-   **Discoverable Keyboard Shortcuts Cheatsheet (T27, 2026-05-01):** A floating "?" trigger button (`#pmg-shortcuts-trigger`, fixed bottom-left, 44×44, z-index 90) opens a dismissible cheatsheet panel (`#pmg-shortcuts-panel`, role=dialog + aria-modal) listing existing shortcuts grouped by area: Global (`?` Open Panel, `/` Search, `Esc` Close), Builder (`G` Generate, `C` Copy), Studio (`S` Focus Studio Textarea), Photo Suite (`R` Surprise Me). Trigger AND `?` toggle are gated by the existing `body.pmg-has-generated` flag (localStorage `pmg_has_generated`) so first-run remains uncluttered. Closes via Esc, × button, or backdrop click; includes a Tab focus trap. The two NEW shortcuts (`S`/`R`) are gated to skip when typing in any input/textarea/contenteditable, when a modifier is held, or when any `[role="dialog"]` / `.pmg-upgrade-overlay` is open. `G`/`C` are NOT re-bound — they remain owned by the existing inline handler in `index.html` (~line 8066) to avoid double-firing; the panel only documents them. Off-screen targets are scrolled into view (smooth, or instant under reduce-motion) before clicking. CSS animations respect `prefers-reduced-motion`. Implementation: new additive `artifacts/promptmegood/public/scripts/pmg-shortcuts.js` plus one script tag in `index.html`. Public API: `window.__pmgShortcuts.{open,close,toggle,isOpen}`. Escape hatch: `?noshortcuts` query param or `localStorage.pmg_shortcuts_disable = "1"`.
