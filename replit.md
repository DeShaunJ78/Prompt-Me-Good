...
*   `pmg-adv-mirror.{css,js}` — Mirrors `<details id="advanced-options">` (Growth Mode / Human Voice / Clarity Boost) inside `#tuning-panel`, injected after `#pmgv3-epic-tuning` so it sits beneath the "Who is this for?" input. Mirror controls have their own IDs (`pmg-adv-mirror-{money,human,clarity}`) — no duplicate IDs in the DOM. Bidirectional sync via change/input listeners + per-original MutationObserver on `checked`. Mirror open state persists in `localStorage['pmg:advmirror:open']`. Disable: `?noadvmirror`, `localStorage.pmg_adv_mirror_disable='1'`.
