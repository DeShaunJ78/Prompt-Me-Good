...
*   **Cache-busters (current):** `pmg-chassis-v3.css` `cv3-38`, `pmg-chassis-v3.js` `cv3-40-vault-settings`, `pmg-visual-studio.{css,js}` `vs-22-photo-accordion`, `pmg-storyboard.css` `sb-7`, `pmg-storyboard.js` `sb-9`, `pmg-auto-boost.{css,js}` `ab-3`, `pmg-ux.js` `cv3-39-photo-hints`. Brand assets `?v=5`. Bump only what changed.
