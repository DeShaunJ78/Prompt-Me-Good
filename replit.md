...
-   **Text Studio Pro · Tab Scroll Sync:** Both "Build A Prompt" and "Transform Text" tab clicks now smooth-scroll the relevant section into view — Transform scrolls the panel, Build scrolls the tab nav (so the form lands in the viewport). Gated on an `opts.scroll` parameter so the initial `mount()` call doesn't auto-scroll on page load. Cache buster: `?v=9-tab-scrollback`.
