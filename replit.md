...
*   **Cache-busters (current):** `pmg-chassis-v3.css` `cv3-48-nav-reset`, `pmg-chassis-v3.js` `cv3-50-gemini-honest`, `pmg-visual-studio.{css,js}` `vs-22-photo-accordion`, `pmg-storyboard.css` `sb-7`, `pmg-storyboard.js` `sb-9`, `pmg-auto-boost.css` `ab-4-ghost-send`, `pmg-auto-boost.js` `ab-3`, `pmg-ux.js` `cv3-39-photo-hints`, `pmg-send-to.js` `sendto-3-gemini-honest`. Brand assets `?v=5`. Bump only what changed.
*   **Send to Gemini:** Verified in a real browser — `gemini.google.com/app` ignores `?q=`, `?text=`, `?prompt=`, and hash variants. Earlier attempts to prefill via URL were a lie (toast said "prefilled" while input stayed empty). Both surfaces (`pmg-send-to.js` Gemini destination, `pmg-chassis-v3.js` `.btn-send-to` handler) now copy-to-clipboard + open Gemini bare and tell the user to paste. ChatGPT (`chatgpt.com/?q=`) and Claude (`claude.ai/new?q=`) DO honor `?q=` — keep those as real prefill.
