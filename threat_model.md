...
PromptMeGood is a production web application with a static frontend in `artifacts/promptmegood/` and an Express API in `artifacts/api-server/`. The backend brokers AI requests to OpenAI, authenticates users with Supabase JWTs or PromptMeGood developer API keys, stores commercial and waitlist data in Supabase/PostgreSQL, and manages paid access through Stripe Checkout and Stripe webhooks.
...
- **Developer API keys** — `pmg_live_*` bearer tokens inherit the owning account's plan and caps. Compromise allows API access as that user until revoked.
...
- **Public to authenticated boundary** — some routes are intentionally anonymous (`/health`, public config, waitlist/contact/founding checkout), while account/profile/billing surfaces require a valid Supabase JWT and developer API traffic requires a valid `pmg_live_*` bearer token. This boundary must be enforced server-side, not inferred from client UI.
...
- **Highest-risk code areas** — `artifacts/api-server/src/routes/ai.ts`, `routes/billing.ts`, `routes/developer.ts`, `routes/founding-checkout.ts`, `routes/stripe-webhook.ts`, `middlewares/apiKeyAuth.ts`, `middlewares/userCaps.ts`, `lib/auth.ts`, `lib/supabase-admin.ts`, `lib/usage-store.ts`.
...
- **Authenticated surfaces** — `/api/create-checkout-session`, `/api/me/profile`, `/api/usage/check`, `/api/developer/*`, and authenticated AI routes that accept Bearer Supabase JWTs or `pmg_live_*` API keys.
...
The application trusts Supabase JWTs and active `pmg_live_*` developer API keys for authenticated API access, and Stripe signatures for webhook state changes. Protected endpoints must require a valid bearer token and bind any returned or mutated data to the authenticated user. API keys must inherit the owning user's plan, owner-bypass status, and daily caps. Webhook processing must only accept Stripe-signed payloads and must not trust user-controllable metadata without additional integrity checks.
...
Public AI-adjacent endpoints, checkout/session creation, contact/waitlist forms, developer API-key traffic, and external-provider calls are all abuse targets. The application must enforce server-side rate limits, per-user/per-key inherited caps, and bounded input sizes so anonymous or low-tier users cannot trigger unbounded model spend, email spam, or expensive external calls.
